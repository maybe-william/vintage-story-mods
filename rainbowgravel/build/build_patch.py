...
// This file is generated by build_patch.py and build.sh within the build/ 
//   directory for the notwilliamrainbowgravel mod.
...
# Filter blacklisted against this list as an additional whitelist
#  in order to further reduce unusable items/blocks.
en_json_whitelist = [
]

def get_en_json_whitelist():
    """Gets the whitelist from the en.json file, which is used for localization 
    and should contain all the items and blocks that are actually "normal" in the game
    with a plain english name."""
    with open("original_en.json", "r") as f:
        data = f.read()
        lines = data.splitlines()
        for line in lines:
                # example of valid line:
            	#   "item-decalcan-alchemy": "Decal can (Alchemy symbols)",
            match = re.match(r'\s*"(item-|block-)([^"]*)":.*', line)
            if match:
                code = match.group(2)
                code = code.replace("*", ".*") # make code into official regex
                en_json_whitelist.append(code)
                print(f"{code} added to en_json_whitelist from original_en.json")
get_en_json_whitelist()

def compile_regexes():
    """Compiles the regexes in the blacklists for faster matching (not fully necessary)"""
    lists = [items_blacklist, blocks_blacklist, items_whitelist, blocks_whitelist, en_json_whitelist]
    for l in lists:
...
def filter(string, item=True, whitelist=False):
    ...
    for pattern in bl:
        if re.match(pattern, string):
            return False
    for pattern in en_json_whitelist:
        if re.match(pattern, string):
            print(f"INFO: Whitelisting from en.json matched: Pattern {pattern} matched string {string}")
            return True
    return False
